function [dist, rmsd, transf, pdb2tx] = pdbsuperpose(PDB1, PDB2, varargin)
...
%   for i = 1:4 
%       [d(i), rmsd(i), tr(i), strtx] = pdbsuperpose(str1, strtx, 'segment', ...
...
%   [d, rmsd, tr] = pdbsuperpose('2trx', '2trx', 'segment', {'A', 'B'}, 'apply', 'chain');
%   
...
if ~start(1)
    ...
    if start(1) < min(res1) || stop(1) > max(res1)
        error(message('bioinfo:pdbsuperpose:InvalidSegmentOne'));
    end
end
...
if ~start(2)
    ...
    if start(2) < min(res2) || stop(2) > max(res2)
        error(message('bioinfo:pdbsuperpose:InvalidSegmentTwo'));
    end
end
